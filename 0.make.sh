...
class LocalChatClient:
    def __init__(self, base_url="http://127.0.0.1:8000"):
        ...
        self.session.headers.update({
            'User-Agent': 'LocalChat-Python-Client/1.0'
        })
    ...
    def login(self):
        ...
        try:
            ...
            if response.status_code == 302:  # 리다이렉트 = 로그인 성공
                self.username = username
    ...
    def get_rooms(self):
        ...
        try:
            ...
            if response.status_code == 200:
                # HTML에서 채팅방 링크 추출
                room_links = re.findall(r'href="/chat/([^"]+)"[^>]*># ([^<]+)', response.text)
                return [(room_key, room_name) for room_key, room_name in room_links]
            else:
    ...
    def show_rooms(self):
        ...
        print("\n=== 채팅방 목록 ===")
        for i, (room_key, room_name) in enumerate(rooms, 1):
            print(f"{i}. {room_name} ({room_key})")
        print("0. 새 채팅방 만들기")
        print("q. 종료")
    ...
    def create_room(self):
        """새 채팅방 만들기"""
        print("\n=== 새 채팅방 만들기 ===")
        room_key = input("방 키 (영문/숫자/._- 3~32자): ").strip()
        room_name = input("방 이름: ").strip()
        
        if not room_key or not room_name:
            print("방 키와 방 이름을 모두 입력하세요.")
            return None
        
        if not re.match(r'^[A-Za-z0-9._-]{3,32}$', room_key):
            print("방 키 형식이 올바르지 않습니다.")
            return None

        try:
            create_data = {"room_key": room_key, "room_name": room_name}
            response = self.session.post(f"{self.base_url}/rooms/create", data=create_data, allow_redirects=False)
            
            if response.status_code == 302:  # 리다이렉트 = 성공
                print(f"채팅방 '{room_name}' ({room_key})이 생성되었습니다.")
                return room_key
            else:
                print("채팅방 생성 실패.")
                return None
    ...
    def get_messages(self, room_key):
        """채팅방 메시지 가져오기"""
        try:
            response = self.session.get(f"{self.base_url}/chat/{room_key}")
            if response.status_code == 200:
                # HTML에서 메시지 추출
                messages = re.findall(r'<b>\[([^\]]+)\]</b> ([^<]+)<span[^>]*> \(([^)]+)\)</span>', response.text)
                return messages
            else:
    ...
    def show_messages(self, room_key, room_name):
        """채팅방 메시지 표시"""
        messages = self.get_messages(room_key)
        if not messages:
            print("메시지가 없습니다.")
            return
        
        print(f"\n=== #{room_name} ({room_key}) ===")
        for username, body, timestamp in messages:
            marker = "나" if username == self.username else f"[{username}]"
            print(f"{marker}: {body} ({timestamp})")

    def send_message(self, room_key, message):
        """메시지 전송"""
        try:
            message_data = {"message": message}
            response = self.session.post(f"{self.base_url}/chat/{room_key}", data=message_data, allow_redirects=False)
    ...
    def chat_room(self, room_key, room_name):
        """채팅방 입장"""
        self.current_room = room_key
        print(f"\n=== #{room_name} ({room_key}) 채팅방 ===")
        print("명령어: /exit (나가기), /refresh (새로고침), /rooms (방 목록)")
        
        while True:
            # 최신 메시지 표시
            self.show_messages(room_key, room_name)
            ...
            if message == "/exit":
                ...
                if self.send_message(room_key, message):
                    print("전송됨")
                else:
                    print("전송 실패")
    ...
    def run(self):
        ...
        while True:
            ...
            if choice == 'q':
                ...
                room_key = self.create_room()
                if room_key:
                    rooms = self.get_rooms()
                    room_name = next((name for key, name in rooms if key == room_key), room_key)
                    self.chat_room(room_key, room_name)
            else:
                try:
                    room_index = int(choice) - 1
                    rooms = self.get_rooms()
                    if 0 <= room_index < len(rooms):
                        room_key, room_name = rooms[room_index]
                        self.chat_room(room_key, room_name)
                    else:
                        print("잘못된 선택입니다.")
                except ValueError:
...
aws_login_env(){
  ...
  echo "DEBUG => AK=$AWS_ACCESS_KEY_ID"
  echo "DEBUG => SK=$AWS_SECRET_ACCESS_KEY"
  echo "DEBUG => RG=$AWS_DEFAULT_REGION"
