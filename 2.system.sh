...
# ====== S3 버킷 지정/자동 생성 옵션 (요청사항 추가) ======
S3_BUCKET=""                 # 비워두면 실행 중 상호작용으로 입력. 미리 정해두려면 'my-backup-bucket' 또는 's3://my-backup-bucket'
S3_REGION="ap-northeast-2"   # 기본 리전
S3_AUTO_CREATE="no"          # yes 로 두면 버킷이 없을 때 자동 생성
S3_ENABLE_VERSIONING="no"    # yes 로 두면 버전닝 활성화
S3_ENABLE_SSE="no"           # yes 로 두면 SSE-S3(AES256) 기본 암호화
S3_LIFECYCLE_DAYS=""         # 숫자(예: 30)로 두면 만료 수명주기 규칙 추가
